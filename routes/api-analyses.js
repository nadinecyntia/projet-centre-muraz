...
// ===== ENDPOINT ANALYSES D'ŒUFS PAR MOIS (PAR SECTEUR) =====
router.get('/analyses/oeufs-mois', async (req, res) => {
    try {
        console.log('📅 API Analyses Œufs par mois appelée');
        ...
        try {
            const query = `
                SELECT
                    DATE_TRUNC('month', eggs_visit_start_date) AS month,
                    unnest(eggs_sector) AS sector,
                    SUM(eggs_count) AS total_eggs
                FROM eggs_collection_new
                WHERE status = 'approved'
                GROUP BY 1, 2
                ORDER BY month ASC, sector ASC;
            `;
            
            const result = await client.query(query);
            
            console.log(`✅ ${result.rows.length} mois d'analyses d'œufs récupérés`);
            
            res.json({
                success: true,
                data: result.rows,
                total: result.rows.length,
                message: 'Données d\'œufs par mois (par secteur) récupérées avec succès'
            });
        ...
        console.error('❌ Erreur API Analyses Œufs par mois:', error);
        res.status(500).json({
            success: false,
            message: 'Erreur lors de la récupération des données d\'œufs par mois',
            error: error.message
        });
    }
});
