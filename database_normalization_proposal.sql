-- =====================================================
-- PROPOSITION DE NORMALISATION RESPECTANT LA LOGIQUE MÉTIER
-- Centre MURAZ - Structure optimisée pour calculs par maison
-- =====================================================

-- =====================================================
-- 1. TABLE DES LOCALISATIONS (Référentiel géographique)
-- =====================================================
...
-- =====================================================
-- 3. TABLE DES BATches DE COLLECTE
-- =====================================================
...
-- =====================================================
-- 4. TABLE DES ESPÈCES (Référentiel taxonomique)
-- =====================================================
...
-- =====================================================
-- 5. TABLE DES COLLECTES ADULTES (4 lignes par maison)
-- =====================================================
CREATE TABLE adult_collections (
    ...
    -- Contrainte d'unicité : une seule ligne par maison/méthode/localisation
    UNIQUE(house_id, collection_method, capture_location, visit_date)
);

-- =====================================================
-- 6. TABLE DES COMPTAGES PAR ESPÈCE (Adultes)
-- =====================================================
...
-- =====================================================
-- 7. TABLE DES GÎTES LARVAIRES (1 ligne par gîte)
-- =====================================================
CREATE TABLE breeding_sites (
    ...
    site_type VARCHAR(50), -- 'pneu', 'bidon', 'bassin', etc.
    site_class VARCHAR(50), -- 'household waste', 'abandoned utensils', etc.
...
-- =====================================================
-- 8. TABLE DES COMPTAGES LARVAIRES/NYMPHES
-- =====================================================
...
-- =====================================================
-- 9. TABLE DES COLLECTES D'ŒUFS (1 ligne = 1 maison)
-- =====================================================
...
CREATE TABLE validations (
    ...
    INDEX idx_validations_type_id (collection_type, collection_id),
    INDEX idx_validations_status (status)
);

-- =====================================================
-- 11. INDEX POUR OPTIMISATION DES REQUÊTES
-- =====================================================
...
-- =====================================================
-- 12. VUES POUR FACILITER LES CALCULS D'INDICES
-- =====================================================

-- Vue pour les calculs d'adultes par maison
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN ac.collection_method = 'prokopack' THEN ac.id END) as prokopack_collections,
    COUNT(DISTINCT CASE WHEN ac.collection_method = 'bg_trap' THEN ac.id END) as bg_trap_collections,
...
-- =====================================================
-- 13. FONCTIONS POUR CALCULER LES INDICES
-- =====================================================

-- Fonction pour calculer l'Indice de Breteau
...
BEGIN
    ...
    FROM houses h
    JOIN locations l ON h.location_id = l.id
    JOIN breeding_sites bs ON h.id = bs.house_id
    WHERE l.sector = p_sector 
    AND l.environment = p_environment
    AND bs.visit_date BETWEEN p_period_start AND p_period_end
...
-- Fonction pour calculer l'Indice de Maison
